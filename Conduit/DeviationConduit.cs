    ...
    public class CurveDeviationCommand : Command
    {
        ...
        protected override Result RunCommand(RhinoDoc doc, RunMode mode)
        {
            ...
            else
            {
                ...
                Viewport.Redraw(doc, "CurveDeviationCommand"); ;

                log.temp("Minimum deviation = {0}   pointA({1}), pointB({2})", min_distance, min_dist_pt_a, min_dist_pt_b);
                log.temp("Maximum deviation = {0}   pointA({1}), pointB({2})", max_distance, max_dist_pt_a, max_dist_pt_b);
            }
